...
function index()
    ...
    -- For mini sys
    entry({"api", "xqsystem", "userdisk_data"}, call("getUserdiskDataInfo"), (""), 194)
    entry({"api", "xqsystem", "backup_data"}, call("backupData"), (""), 196)
    entry({"api", "xqsystem", "backup_status"}, call("backupStatus"), (""), 197)
    entry({"api", "xqsystem", "backup_cancel"}, call("backupCancel"), (""), 198)
end
...
function getUserdiskDataInfo()
    local XQSysUtil = require("xiaoqiang.util.XQSysUtil")
    local result = {}
    local prepare = XQSysUtil.diskPrepare()
    if prepare == 0 then
        result = XQSysUtil.getDirectoryInfo()
        result.code = 0
        result.mount = XQSysUtil.mountInfo()
    elseif prepare == 1 then
        result.code = 1622
...
function backupData()
    ...
    if files then
        ...
        os.execute("echo 1 prepare > /tmp/backup_files_status")
