...
function write(id, data)
    ...
    if not _checkid(id) then
        OKLog.log(3,"Security Exception: Session ID is invalid! sauth.write")
        return
    end

    if type(data) ~= "table" then
        OKLog.log(3,"Security Exception: Session data invalid! sauth.write")
        return
    end
...
function read(id)
    ...
    if not _checkid(id) then
        OKLog.log(3,"Security Exception: Session ID is invalid! sauth.read")
        return nil
    end
    ...
    if type(sess) ~= "table" then
        OKLog.log(3,"Security Exception: Session data invalid! sauth.read")
        return nil
    end
...
function kill(id)
    if not _checkid(id) then
        OKLog.log(3,"Security Exception: Session ID is invalid! sauth.kill")
    else
