...
function error500(message)
    ...
    local logger = require("officekit.OKLog")
log.print(message)
...
function empower(lan,wan,admin)
    ...
    local mac = OKFunction.macFormat(luci.sys.net.ip4mac(remoteAddr))
    if not OKFunction.isStrNil(mac) then
        local OKSysUtil = require("officekit.util.OKSysUtil")
        if not OKSysUtil.setMacFilter(mac,nil,wan,admin) then
            local OKLog = require("officekit.OKLog")
            OKLog.log(3,"Empower failed"..mac)
        end
    end
end
...
function authenticator.jsonauth(validator, accs, default)
    ...
    local nonce = luci.http.formvalue("nonce")
    local uuid = luci.http.formvalue("uuid")
    local token = luci.http.formvalue("token")
    if nonce then
        if OKSecureUtil.checkNonce(nonce, getremotemac()) then
            if OKSecureUtil.checkUser(user, nonce, pass) then
                empower("1","1",nil)
                local logtype = "2"
...
function authenticator.htmlauth(validator, accs, default)
    local OKSysUtil = require("officekit.util.OKSysUtil")
    local redirectKey = luci.http.formvalue("redirectKey")

    if redirectKey then
        --local check = OKSecureUtil.checkRedirectKey(redirectKey)
		local check = 2
        if check then
            if check == "1" then
                if true then
                    luci.http.header("Set-Cookie", "psp=" .. bindUUID .. "|||1|||dfadfqewrrq;path=/;")
    ...
    require("luci.template")
    context.path = {}
    luci.template.render("okweb/sysauth", {duser=default, fuser=user})
    return false
end
...
function authenticator.htmlauth_moblie(validator, accs, default)
    ...
    local nonce = luci.http.formvalue("nonce")
    if nonce then
        if OKSecureUtil.checkNonce(nonce, getremotemac()) and OKSecureUtil.checkUser(user, nonce, pass) then
            empower("1","1",nil)
            return user, "2"
        end
    end
...
function check_show_syslock(sysauth)
    ...
    if OKFunction.sysLockStatus() == 1 then
        if OKSysUtil.isUpgrading() then
            require("luci.i18n")
            require("luci.template")
            if type(sysauth) == "string" and sysauth == "htmlauth" then
                 context.path = {}
                 -- luci.template.render("okweb/syslock",{})
...
function http_request_log(request, tag)
    ...
    if requestUri and tag and type(tag) == "string" then
        ...
        OKLog.log(6,tag..":"..uriInfo[1])
        if uriInfo[2] then
            OKLog.log(7,uriInfo[2])
        end
    end
end
...
function dispatch(request)
    ...
    if track.sysauth and track.noauth~=true and remoteaddr ~= "127.0.0.1" then
        ...
        local crypto = require "officekit.util.OKCryptoUtil"
