module ("officekit.util.OKLanWanUtil", package.seeall)
...
function getDefaultMacAddress()
    ...
    local mac = LuciUtil.exec(OKConfigs.GET_DEFAULT_MACADDRESS)
    if OKFunction.isStrNil(mac) then
        mac = nil
    else
...
function getWanMonitorStat()
    ...
    local content = NixioFs.readfile(OKConfigs.WAN_MONITOR_STAT_FILEPATH)
    local status = {}
    if content ~= nil then
...
function _pppoeStatusCheck()
    ...
    if status then
        ...
        if OKFunction.isStrNil(status) then
            return false
        end
...
function getPPPoEStatus()
    ...
    if status then
        ...
        if status.proto == "pppoe" then
            ...
            if not OKFunction.isStrNil(configdns) then
...
function pppoeStart()
    OKFunction.forkExec("lua /usr/sbin/pppoe.lua up")
end
...
function getLanWanInfo(interface)
    ...
    if network then
        ...
        if interface == "wan" then
            ...
            if OKFunction.isStrNil(mtuvalue) then
                mtuvalue = "1480"
            end
            local special = network:get_option_value("special")
            if OKFunction.isStrNil(special) then
                special = 0
            end
            ...
            if not OKFunction.isStrNil(data) then
...
function getWanMac()
    ...
    if not OKFunction.isStrNil(ifconfig) then
...
function setLanIp(ip,mask)
    local OKEvent = require("officekit.OKEvent")
    local LuciNetwork = require("luci.model.network").init()
...
function wanRestart()
    ...
    OKFunction.forkExec("/etc/init.d/filetunnel restart")
end
...
function getWanDetails()
    ...
    if wanNetwork then
        ...
        if not OKFunction.isStrNil(wanNetwork:get_option_value("dns")) then
...
function generateDns(dns1, dns2, peerdns)
    local dns
    if not OKFunction.isStrNil(dns1) and not OKFunction.isStrNil(dns2) then
        dns = {dns1,dns2}
    elseif not OKFunction.isStrNil(dns1) then
        dns = dns1
    elseif not OKFunction.isStrNil(dns2) then
        dns = dns2
    end
    return dns
end
...
function setWanPPPoE(name, password, dns1, dns2, peerdns, mtu, special, service)
    ...
    if oldconf.username == name
        ...
        and ((OKFunction.isStrNil(oldconf.service) and OKFunction.isStrNil(service)) or oldconf.service == service)
    ...
    if not OKFunction.isStrNil(dns1) then
        table.insert(dnss, dns1)
    end
    if not OKFunction.isStrNil(dns2) then
    ...
    if not OKFunction.isStrNil(name) then
        OKPreference.set(OKConfigs.PREF_PPPOE_NAME,name)
        OKFunction.nvramSet("nv_pppoe_name", name)
    end
    if not OKFunction.isStrNil(password) then
        OKPreference.set(OKConfigs.PREF_PPPOE_PASSWORD,password)
        OKFunction.nvramSet("nv_pppoe_pwd", password)
    end
    OKFunction.nvramSet("nv_wan_type", "pppoe")
    OKFunction.nvramCommit()
    if wanNet then
        LuciNetwork:save("network")
        LuciNetwork:commit("network")
        --os.execute(OKConfigs.VPN_DISABLE)
        if dnsrestart then
            dnsmsqRestart()
        end
        if wanrestart then
...
function setWanStaticOrDHCP(ipType, ip, mask, gw, dns1, dns2, peerdns, mtu)
    ...
    OKFunction.nvramSet("nv_wan_type", ipType)
    OKFunction.nvramCommit()
    if wanNet then
        LuciNetwork:save("network")
        LuciNetwork:commit("network")
...
function getWanSpeed()
    local LuciUtil = require("luci.util")
    local OKPreference = require("officekit.OKPreference")
    local wanspeed = tonumber(OKPreference.get("WAN_SPEED", 0))
...
function setWanSpeed(speed)
    ...
    if speed then
        OKPreference.set("WAN_SPEED", speed)
        if speed == 0 then
...
function pppoeCatch(timeout)
    ...
    if OKFunction.isStrNil(result.pppoename) and OKFunction.isStrNil(result.pppoepasswd) then
        result.code = 1
    end
    return result
end
