module ("officekit.util.OKHttpUtil", package.seeall)
...
function httpGetRequest(url, paramStr, cookies)
    ...
    if cookies and type(cookies) == "table" then
        ...
        for key,value in pairs(cookies) do
            cookieStr = cookieStr..key.."="..value..";path=/;domain=.officekit.org;"
        end
    ...
    if OKFunction.isStrNil(cookieStr) then
        if OKFunction.isStrNil(paramStr) then
            res, code, headers, status = httpHandler.request(url)
        else
            res, code, headers, status = httpHandler.request(url, paramStr)
        end
    else
        if not OKFunction.isStrNil(paramStr) then
