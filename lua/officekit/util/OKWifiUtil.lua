module ("officekit.util.OKWifiUtil", package.seeall)
...
function getWifiConnectDeviceList(wifiIndex)
    ...
    if wifiUp then
        for mac, info in pairs(assoclist) do
            table.insert(dlist, OKFunction.macFormat(mac))
        end
    end
    return dlist
end
...
function isDeviceWifiConnect(mac,wifiIndex)
    ...
    if type(dict) == "table" and #dict>0 then
        return dict[OKFunction.macFormat(mac)] ~= nil
    else
        return false
    end
end
...
function getWifiConnectDeviceDict(wifiIndex)
    ...
    if wifiUp then
        for mac, info in pairs(assoclist) do
            if mac then
                dict[OKFunction.macFormat(mac)] = 1
            end
        end
    end
    return dict
...
function getWifiWorkChannel(wifiIndex)
    local channel = ""
    if tonumber(wifiIndex) == 1 then
        channel = LuciUtil.trim(LuciUtil.exec(OKConfigs.WIFI24_WORK_CHANNEL))
    else
        channel = LuciUtil.trim(LuciUtil.exec(OKConfigs.WIFI50_WORK_CHANNEL))
    end
...
function getWifiDeviceSignalDict(wifiIndex)
    ...
    for mac, info in pairs(assoclist) do
        if mac then
            result[OKFunction.macFormat(mac)] = 2*math.abs(tonumber(info.signal)-tonumber(info.noise))
        end
    end
    return result
end
...
function getAllWifiConnetDeviceDict()
    ...
    for index = 1,2 do
        ...
        for _, mac in pairs(wifilist) do
            ...
            result[OKFunction.macFormat(mac)] = item
        end
    end
    return result
end
...
function channelFormat(wifiIndex, channel, bandwidth)
    ...
    if channelList and type(channelList) == "table" then
        ...
        if not OKFunction.isStrNil(channel) then
            return channel
        end
    end
    return false
...
function setWifiChannelTxpwr(channel1,txpwr1,channel2,txpwr2)
    ...
    if wifiDev1 then
        ...
        if not OKFunction.isStrNil(txpwr1) then
    ...
    if wifiDev2 then
        ...
        if not OKFunction.isStrNil(txpwr2) then
...
function setWifiTxpwr(txpwr)
    ...
    local wifiDev2 = network:get_wifidev(LuciUtil.split(_wifiNameForIndex(2),".")[1])
    if wifiDev1 then
        if not OKFunction.isStrNil(txpwr) then
...
function checkWifiPasswd(passwd,encryption)
    if OKFunction.isStrNil(encryption) or (encryption and encryption ~= "none" and OKFunction.isStrNil(passwd)) then
        return 1502
    end
...
function checkSSID(ssid,length)
    if OKFunction.isStrNil(ssid) then
        return 0
    end
    if string.len(ssid) > tonumber(length) then
        return 1572
    end
    if not OKFunction.checkSSID(ssid) then
        return 1573
    end
    return 0
end
...
function setWifiBasicInfo(wifiIndex, ssid, password, encryption, channel, txpwr, hidden, on, bandwidth)
    ...
    if wifiDev then
        ...
        if not OKFunction.isStrNil(bandwidth) then
    ...
    if not OKFunction.isStrNil(ssid) then
...
function setWifiRegion(country, region, regionABand)
    if OKFunction.isStrNil(country) or not tonumber(region) or not tonumber(regionABand) then
        return false
    end
    local network = LuciNetwork.init()
    local wifiDev1 = network:get_wifidev(LuciUtil.split(_wifiNameForIndex(1),".")[1])
...
function wifiBridgedClientId()
    ...
    for i, wifiDev in ipairs(wifiDevs) do
        ...
        if not OKFunction.isStrNil(clientId) then
...
function deleteWifiBridgedClient(wifiIndex)
    ...
    local clientId = wifiStaClientId(wifiIndex)
    local wifiDev = LuciNetwork:get_wifidev(LuciUtil.split(_wifiNameForIndex(wifiIndex),".")[1])
    if not wifiDev and not OKFunction.isStrNil(clientId) then
        return false
    end
...
function setWifiBridgedClient(wifiIndex,ssid,encryption,enctype,key,channel)
    ...
    if OKFunction.isStrNil(clientId) then
        local network = {
            ifname      = wlanX,
            ssid        = ssid,
            mode        = "sta",
...
function getWifiWpsStatus()
    local LuciUtil = require("luci.util")
    local status = LuciUtil.exec(OKConfigs.GET_WPS_STATUS)
...
function getWpsConDevMac()
    local LuciUtil = require("luci.util")
    local mac = LuciUtil.exec(OKConfigs.GET_WPS_CONMAC)
    if mac then
        return OKFunction.macFormat(LuciUtil.trim(mac))
    end
    return nil
end
...
function openWifiWps()
    ...
    LuciUtil.exec(OKConfigs.OPEN_WPS)
    local timestamp = tostring(os.time())
    OKPreference.set(OKConfigs.PREF_WPS_TIMESTAMP,timestamp)
    return timestamp
end
...
local WIFI_LIST_CMD = [[iwpriv ra1 set SiteSurvey=1;iwpriv ra1 get_site_survey | awk '{print $2"|||"$3"|||"$4"|||"$5}']]

function _parseEncryption(encryption)
    if OKFunction.isStrNil(encryption) then
        return nil
    end
...
function _wifiScan()
    local LuciUtil = require("luci.util")
    local result = {}
    local scan = "iwpriv ra0 get_site_survey"
    local linenum = 0
    local channel = 0
    local ssid = ""
    ...
    if scanlist then
	   ...
	   linenum = linenum + 1
	   if linenum % 3 == 0 and not OKFunction.isStrNil(line) then
	   	channel = string.sub(line, 1, 4):match("(%d+)")
	   end 
	   if linenum % 3 == 1 and not OKFunction.isStrNil(line) then
           	ssid =  string.sub(line, 1, 33):match("(%S+)")
	   end
	   if linenum % 3 == 2 and not OKFunction.isStrNil(line) and #line >= 78 then
                --local channel = string.sub(line, 1, 4):match("(%d+)")
                --local ssid = string.sub(line, 5, 37):match("(%S+)")
                local mac = string.sub(line, 1, 20):match("(%S+)")
                local security = string.sub(line, 21, 43):match("(%S+)")
                local signal = string.sub(line, 44, 49):match("(%S+)")
                local wmode = string.sub(line, 50, 56):match("(%S+)")
                local extch = string.sub(line, 57, 63):match("(%S+)")
                local nt = string.sub(line, 64, 66):match("(%S+)")
                local xm = string.sub(line, 67, 72):match("(%S+)") or ""
                local wps = string.sub(line, 73, 76):match("(%S+)")
...
function getWifiScanlist(sitesurvey)
    ...
    if OKFunction.isStrNil(sitesurvey) then
        scan = "iwpriv ra0 set SiteSurvey=;sleep 1"
    else
        scan = "iwpriv ra0 set SiteSurvey=\""..sitesurvey.."\";sleep 1"
    end
    os.execute(scan)
    for i=1, 1 do
...
function addDevice(model, mac, name)
    ...
    if not OKFunction.isStrNil(mac) and not OKFunction.isStrNil(name) then
        mac = OKFunction.macFormat(mac)
        OKDBUtil.saveDeviceInfo(mac, name, name, "", "")
        local uci = require("luci.model.uci").cursor()
        local config = tonumber(model) == 0 and "wifiblist" or "wifiwlist"
        local maclist = uci:get_list(config, "maclist", "mac") or {}
...
function editWiFiMacfilterList(model, macs, option)
    if not macs or type(macs) ~= "table" or OKFunction.isStrNil(option) then
        return 2
    end
    local uci = require("luci.model.uci").cursor()
    local config = tonumber(model) == 0 and "wifiblist" or "wifiwlist"
    ...
    if option == 0 then
        local macdic = {}
        for _, macaddr in ipairs(maclist) do
            macdic[OKFunction.macFormat(macaddr)] = 1
        end
        for _, macaddr in ipairs(macs) do
            if not OKFunction.isStrNil(macaddr) then
                macdic[OKFunction.macFormat(macaddr)] = 1
            end
        end
        maclist = {}
        ...
        for _, macaddr in ipairs(macs) do
            if not OKFunction.isStrNil(macaddr) then
                macdic[OKFunction.macFormat(macaddr)] = 0
            end
        end
        maclist = {}
        for mac, value in pairs(macdic) do
    ...
    local macstr = OKFunction._cmdformat(table.concat(maclist, ";"))
    os.execute("iwpriv ra0 set ACLClearAll=1")
    os.execute("iwpriv ra1 set ACLClearAll=1")
    os.execute("iwpriv ra0 set ACLAddEntry=\""..macstr.."\"")
    os.execute("iwpriv ra1 set ACLAddEntry=\""..macstr.."\"")

    if tonumber(model) == 0 then
        os.execute("iwpriv ra0 set AccessPolicy=2")
        os.execute("iwpriv ra1 set AccessPolicy=2")
    else
        os.execute("iwpriv ra0 set AccessPolicy=1")
        os.execute("iwpriv ra1 set AccessPolicy=1")
    end
    if #maclist > 0 then
        uci:set_list(config, "maclist", "mac", maclist)
    else
        uci:delete(config, "maclist", "mac")
        os.execute("iwpriv ra0 set AccessPolicy=0")
        os.execute("iwpriv ra1 set AccessPolicy=0")
    end
    uci:commit(config)
    -- wireless
    local macfilter
...
function getWiFiMacfilterInfo(model)
    ...
    local LuciNetwork = require("luci.model.network").init()
    local OKDBUtil = require("officekit.util.OKDBUtil")
    local OKEquipment = require("officekit.OKEquipment")
    local wifiNet = LuciNetwork:get_wifinet(_wifiNameForIndex(1))
    local info = {
        ["enable"] = 0,
        ["model"] = 0
    ...
    for _, mac in ipairs(mlist) do
        ...
        local name = ""
        local device = OKDBUtil.fetchDeviceInfo(mac)
        if device then
            local originName = device.oName
            local nickName = device.nickname
            if not OKFunction.isStrNil(nickName) then
                name = nickName
            else
                local company = OKEquipment.identifyDevice(mac, originName)
                local dtype = company["type"]
                if OKFunction.isStrNil(name) and not OKFunction.isStrNil(dtype.n) then
                    name = dtype.n
                end
                if OKFunction.isStrNil(name) and not OKFunction.isStrNil(originName) then
                    name = originName
                end
                if OKFunction.isStrNil(name) and not OKFunction.isStrNil(company.name) then
                    name = company.name
                end
                if OKFunction.isStrNil(name) then
                    name = mac
                end
                if dtype.c == 3 and OKFunction.isStrNil(nickName) then
                    name = dtype.n
                end
            end
            item["name"] = name
...
function scanWifiChannel(wifiIndex)
    ...
    local wifi = tonumber(wifiIndex) == 1 and "ra1" or "ra0"
    local scancmd = "iwpriv "..tostring(wifi).." ScanResult"
    local scanresult = LuciUtil.exec(scancmd)
    if not OKFunction.isStrNil(scanresult) then
...
function wifiChannelQuality()
    ...
    if wifiinfo[1] and wifiinfo[1].status == "1" then
        OKFunction.forkExec("sleep 4; iwpriv ra1 set AutoChannelSel=3")
    end
    if wifiinfo[2] and wifiinfo[2].status == "1" then
        OKFunction.forkExec("sleep 4; iwpriv ra0 set AutoChannelSel=3")
    end
end

function iwprivSetChannel(channel1, channel2)
    if tonumber(channel1) then
        local setcmd = "sleep 4; iwpriv ra1 set channel="..tostring(channel1)
        OKFunction.forkExec(setcmd)
    end
    if tonumber(channel2) then
        local setcmd = "sleep 4; iwpriv ra0 set channel="..tostring(channel2)
        OKFunction.forkExec(setcmd)
    end
end
