...
(function ($) {
    ...
    var defaults = {
        messages: i18n.validate,
        callback: function(errors, event) {
            var root = this.form;
            $('.form-item', root).removeClass('form-item-err');
            $('.form-item .t', root).hide();

            if (errors.length > 0) {
                for (var i = 0; i < errors.length; i++) {
                    var item = $(root[errors[i].name]).parents('.form-item');
                    $(item).addClass('form-item-err');
                    $('.t', item).html(errors[i].message).show();
                };
            };
        }
    };
    ...
    */
        ...
        for (var i = 0, ruleLength = rules.length; i < ruleLength; i++) {
            ...
            if (failed) {
                ...
                var source = field.msg[method] || this.messages[method] || defaults.messages[method],
                    message = field.display + defaults.messages['default'];
    ...
    FormValidator.prototype._hooks = {
        ...
        valid_weppassword: function(field){
